...
public class MazeGenerator : MonoBehaviour
{
    ...
    {
        ClearMaze();
        ClearEnemies(); // Очищення ворогів при створенні нового лабіринту
        ...
        GameObject exit = Instantiate(exitPrefab, new Vector2(farthestCell.x, farthestCell.y), Quaternion.identity);
        ...
        if (player != null)
        {
            player.transform.position = new Vector2(start.x, start.y);
        }
    }
    ...
    void SpawnEnemies()
    {
        int enemyCount = currentLevel * 4; // Кількість ворогів залежить від рівня
    ...
    void UpdateLevelText()
    {
        if (levelText != null)
        {
            ...
            levelText.fontSize += 0.3f; // Збільшення розміру шрифту на 0.5 при кожному новому рівні
        }
    }
